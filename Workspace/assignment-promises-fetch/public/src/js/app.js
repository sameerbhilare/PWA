...
button.addEventListener('click', function () {
  ...
  var promise = new Promise(function (resolve, reject) {
    setTimeout(function () {
      ...
      // Resolve the following URL: https://reqres.in/api/users/2
      resolve('https://reqres.in/api/users/2');
    }, 2000);
  })
    .then(function (url) {
      return fetch(url);
    ...
    .then(function (response) {
      output.textContent = response.data.first_name + ' ' + response.data.last_name;
    })
    .catch(function (err) {
      console.log(err);
    });
